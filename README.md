...
使用：屏蔽词的增删取决于您，如果您的bot对用户输入信息安全性有较高要求（例如使用官方bot），则推荐您使用该插件。

自定义：本插件一切数据均可自定义，会根据配置规则来回复，针对不同恶意用户进行不同等级回复与屏蔽。

文字狱：为了避免文字狱情况，本插件具有超管可自行增删屏蔽词的功能，请善用指令。

停用词库：如果您不希望某个类型屏蔽词库生效，请直接删除词库文件。若需修改回怼规则，请自行修改配置去除。

声明：该项目仅供学习交流，维护互联网环境健康和谐，违禁词来自已有项目。任非法滥用本项目所造成的问题本插件概不负责。
